...
import UpdateExecComity from './UpdateExecComity';

const SearchExecComityForm = () => {
  const [sno, setSno] = useState(''); // State to manage the input value
  ...
  const handleSearch = () => {
    if (!sno) {
      ...
      setSearchedId(sno); // Set the searched ID
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: 'auto', marginTop: '20px', marginLeft: '20px' }}>
      ...
      <input
        type="text"
        value={sno}
        onChange={(e) => setSno(e.target.value)} // Update state on change
      ...
      <br />
      {searchedId && <UpdateExecComity sno={searchedId} />} {/* Render form if searchedId is set */}
    </div>
  );
};

export default SearchExecComityForm;