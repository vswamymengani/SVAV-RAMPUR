...
const UpdateExecComity = (props) => {
  ...
  useEffect(() => {
    if (sno) {
      ...
      const fetchStudentData = async () => {
        try {
          const response = await axios.get(`http://18.60.190.183:3001/api/committee_members/${sno}`);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axios.put(`http://localhost:3001/api/committee_members/${sno}`, data);
