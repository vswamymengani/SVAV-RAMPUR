...
const UpdateStudentForm = (props) => {
  ...
  useEffect(() => {
    if (stdadmid) {
      ...
      const fetchStudentData = async () => {
        try {
          const response = await axios.get(`http://18.60.190.183:3001/api/students/${stdadmid}`);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axios.put(`http://18.60.190.183:3001/api/students/${stdadmid}`, data);
